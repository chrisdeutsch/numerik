...
\end{align}
folgt. Mit steigender Frequenz $\omega$ geschieht der Ladungstransport überwiegend am Rand des Leiter. Dies geschieht aufgrund des (alternierenden) Magnetfelds, das vom Wechselstrom induziert wird. Nach der Lenz'schen Regel wirkt dieses seiner Ursache entgegen und übt eine Kraft (Lorentzkraft) auf die Ladungsträger aus, die zum Rand des Leiters gerichtet ist. Am Rand sind also mehr Ladungsträger lokalisiert als im Kern, wobei der Ring am Rand des Leiters, in dem sich die Ladungsträger konzentrieren, mit zunehmender Frequenz immer dünner wird und das Phänomen daher als \emph{Skin-Effekt} bezeichnet wird. Insbesondere ist $j_0$ eine komplexe Zahl, da sie nach dem Separationsansatz sowohl Amplituden- wie auch Phaseinformation enthält.
...
Wir haben eine globale Konstante \texttt{kEpsilon} definiert, die als Abbruchbedingung der Reihenberechnung dient. Die Reihen werden dann abgebrochen, wenn der letzte Summand kleiner als \texttt{kEpsilon} multipliziert mit der aktuellen Partialsumme ist. Diese Bedingung wurde gewählt, da die Addition von sehr kleinen Gleitkommazahlen auf sehr Große keine Änderung der Summe erzielt. Mit dieser Abbruchbedingung vermeiden wir unnötige Rechenschritte die das Ergebnis nicht ändern.
