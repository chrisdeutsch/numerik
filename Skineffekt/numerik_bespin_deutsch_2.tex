...
Man sieht leicht, dass diese Methode nicht für $f_0$ und $g_0$ sowie deren Ableitungen funktioniert, da ein Sinus bzw. Cosinus mit $k$-Abhängigkeit im Summanden vorkommt.
Um dennoch die Vorteile der Methode zu erhalten, können wir die problematische Funktion $f(x,k)$ ausklammern (also $S_k = f(x,k) \cdot \alpha_k(x,k)$ sowie $S_{k-1} = f(x,k-1) \cdot \alpha_{k-1}(x,k)$)
und analog zur Methode von vorhin den Faktor $\alpha_k(x,k)$ entwickeln. Dies nimmt dann die allgemeine Form $\alpha_k(x,k) = c(x,k)\cdot\alpha_{k-1}(x,k)$.
