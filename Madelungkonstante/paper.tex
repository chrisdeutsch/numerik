...
\end{tabular}
\captionof{table}{Anzahl der hinzugefügten Schalen $n$ gegen Madelung-Konstante $\tilde{\alpha}(n)$ und relativer Fehler $\delta$}
...
gegen den Wert der Madelung-Konstante $\alpha$ und den relativen Fehler $\delta$ zum
...
sehr klein.
Wie man sieht (Abbildung \ref{plotkonvergenz3d}) alterniert der berechnete Wert
der Madelung-Konstante mit steigenden $n$ und fallendem absoluten Fehler $\Delta$ um
den wahren Wert. Im Folgenden bezeichnet $\alpha$ den wahren Wert der Madelung-Konstante
und $\tilde{\alpha}(n)$ die nach unserem Algorithmus berechnete Madelung-Konstante
nach $n$-Iterationen. Der absolute Fehler ist also:
\begin{align}
...
Abbruchsbedingung der absolute Fehler $\Delta$ kleiner als $\epsilon$ ist.
