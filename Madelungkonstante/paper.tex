...
Der nach unserer Methode berechnete Wert alterniert, wie man in der Abbildung
sehen kann, stets mit fallender Amplitude um den wahren Wert. Dies
rechtfertigt unser Abbruchkriterium, denn der wahre Wert liegt offenbar
näher am letzten Datenpunkt als die Differenz zum vorletzten. So ist garantiert, dass bei
Eintreten der Abbruchsbedingung der absolute Fehler des berechneten Wertes kleiner 
als $\epsilon$ ist.
...
dass um elektrische Neutralität einer Elementarzelle zu erreichen, die Ecken mit $\frac{1}{4}$ und die Kanten mit $\frac{1}{2}$ gewichtet werden müssen (Abbildung \ref{skalierungsgrafik2d}).
\\
\\
