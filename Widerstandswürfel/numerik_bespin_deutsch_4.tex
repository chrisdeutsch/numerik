...
\begin{align}
	A^{(k)} = \prod_{l=k+1}^N \left[ Q^{k}_l \left( -\frac{(A^{(k-1)})_{l,k}}{(A^{(k-1)})_{k,k}}\right)\right] A^{(k-1)} 
\end{align}
...
\label{pivotisierung}
Das in \ref{lr-zerlegung} erklärte Verfahren zur Berechnung der LR-Zerlegung funktioniert nicht für jede invertierbare Matrix (z.B. wenn ein Diagonalelement null ist). Außerdem ist im Allgemeinen das Problem nicht optimal konditioniert. Da wir in einem Gleichungssystem ohne Probleme die Reihenfolge der einzelnen Gleichungen vertauschen können, können wir für eine gute Konditionierung des Gaußverfahrens sorgen.
